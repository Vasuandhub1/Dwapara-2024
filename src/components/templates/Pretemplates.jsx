...
import img from "./Work Together Image.svg"
...
export default function Pretemplates({val}) {
  return (
    ...
    <div id="wrapper" className="flex justify-evenly box-border font-mono ">
      <div className=" border-4  p-4  flex flex-wrap  w-fit justify-center overflow-hidden rounded-xl">
        <div className='w-[23rem] h-[13rem] space-x-5'>
            <h1 className="text-xl">{val.name}</h1>
            <sup className=" text-red-500 font-extrabold p-1 rounded-xl relative top-1">{val.stream}</sup>
            <p className=" overflow-auto w-[20rem] h-[5rem]">{val.description}</p>
